...
import {
  HqFollowUpProvider,
  HqLeadFollowUpControls,
  HqLeadFollowUpTableEditor,
} from "@/components/hq/HqLeadFollowUpControls";
...
function LeadMobileCard({ lead }: { lead: LeadAssessmentRow }) {
  ...
  return (
    <article className="grid gap-4 rounded-card border border-border bg-surface/90 p-4 shadow-card">
      ...
      <HqLeadFollowUpControls lead={lead} />
      ...
      >
        ...
        <div className="hidden overflow-x-auto md:block">
          <table className="w-full min-w-[86rem] border-separate border-spacing-0 text-left text-sm">
            ...
            <tbody>
              {leads.map((lead) => (
                <Fragment key={lead.assessment_id}>
                  <tr className="align-top text-muted transition hover:bg-background/50">
                    ...
                    <td className="w-56 border-b border-border/70 px-3 py-4 pr-0">
