...
| `whatsapp_welcome_sent_at` | `customers.whatsapp_welcome_sent_at` | `2026-07-01T15:25:00Z` | Records when the welcome template was sent. Remains null while sending is disabled or skipped. | System-generated | Yes | Keep as communication audit history while contact data exists. | Communication history/status. |
| `whatsapp_welcome_status` | `customers.whatsapp_welcome_status` | `sent`, `skipped`, `failed` | Records the latest safe server-side welcome template outcome. | System-generated | Yes | Keep as communication audit history while contact data exists. | Communication history/status. |
| `whatsapp_welcome_error` | `customers.whatsapp_welcome_error` | `missing_config` | Stores a minimal non-secret welcome send error or skip reason. | System-generated | Yes | Keep only while useful for operational troubleshooting; avoid storing request payloads or Meta response bodies. | Operational diagnostic linked to communication history. |
...
| `lead_status` | `assessments.lead_status` | `follow_up` | Internal lead follow-up state. | HQ admin entered/generated | Yes | Same as assessment data until a retention policy exists. | Disclosure note: internal customer relationship/support data. |
| `lead_priority` | `assessments.lead_priority` | `high` | Internal lead follow-up priority. | HQ admin entered/generated | Yes | Same as assessment data until a retention policy exists. | Disclosure note: internal customer relationship/support data. |
| `internal_note` | `assessments.internal_note` | `Needs quote for fridge backup.` | Private HQ-only lead note for internal follow-up. | HQ admin entered/generated | Yes | Same as assessment data until a retention policy exists. | Disclosure note: internal customer relationship/support data. |
| `last_contacted_at` | `assessments.last_contacted_at` | `2026-07-08T15:30:00Z` | Records when Daniel last contacted the lead. | HQ admin entered/generated | Yes | Same as assessment data until a retention policy exists. | Disclosure note: internal customer relationship/support data. |
| `follow_up_at` | `assessments.follow_up_at` | `2026-07-10T14:00:00Z` | Planned internal follow-up date/time. | HQ admin entered/generated | Yes | Same as assessment data until a retention policy exists. | Disclosure note: internal customer relationship/support data. |
| `lead_updated_at` | `assessments.lead_updated_at` | `2026-07-08T15:35:00Z` | Records the last internal lead follow-up update time. | HQ admin entered/generated | Yes | Same as assessment data until a retention policy exists. | Disclosure note: internal customer relationship/support data. |
